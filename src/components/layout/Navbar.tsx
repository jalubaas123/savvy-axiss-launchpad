      ...
      duration: 0.5
        ...
        <nav className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 bg-primary-foreground">
          <div className="flex items-center justify-between h-16 sm:h-20 md:h-24 lg:h-28">
            ...
            {/* Center Section: Navigation Menu — nudged right for better balance */}
            <nav className="hidden lg:flex items-center gap-3 xl:gap-4 absolute left-1/2 -translate-x-1/2 lg:ml-5 xl:ml-6">
              {navLinks.map(link => (
                <Link
                  key={link.name}
                  to={link.href}
            ...
            {/* Right Section: CTA Button / Mobile Menu — matching left space so nav stays centered */}
            <div className="flex items-center gap-3 flex-shrink-0 lg:ml-8 xl:ml-10">
              ...
              <button
                ...
                className="lg:hidden p-1.5 sm:p-2 text-foreground hover:text-secondary transition-colors"
